...
class GaussianPositionalEncoding(nn.Module):
    def __init__(self, sigma=25, in_features=3, encoding_dimension=93, use_only_sin=True):
        super().__init__()
        self._use_only_sin = use_only_sin
        if not use_only_sin:
            encoding_dimension = encoding_dimension // 2
        self._b_encoding_matrix = nn.Linear(in_features, encoding_dimension, bias=False)
