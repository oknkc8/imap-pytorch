...
class NERF(nn.Module):
    ...
    def loss(self, batch, reduction=True):
        ...
        if self._cfg.model.loss_type == 'l1':
            coarse_image_loss = torch.mean(self._loss(output['coarse_color'] * max_depth_mask, batch["color"] * max_depth_mask), dim=1)
            ...
            fine_image_loss = torch.mean(self._loss(output['fine_color'] * max_depth_mask, batch["color"] * max_depth_mask), dim=1)
            ...
            coarse_image_loss = torch.mean(self._mseloss(output['coarse_color'] * max_depth_mask, batch["color"] * max_depth_mask), dim=1)
            coarse_depth_loss = self._mseloss(output['coarse_depth'] * mask, batch["depth"] * mask) * valid_weight

            fine_image_loss = torch.mean(self._mseloss(output['fine_color'] * max_depth_mask, batch["color"] * max_depth_mask), dim=1)
